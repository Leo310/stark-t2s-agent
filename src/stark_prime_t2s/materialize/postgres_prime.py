...
class PostgresPrimeStore:
    ...
    def _insert_node_batch(self, type_name: str, rows: list) -> None:
        ...
        with self.engine.connect() as conn:
            conn.execute(
                text(f"INSERT INTO {table_name} (id, name, description, raw_json) "
                     f"VALUES (:id, :name, :description, :raw_json) "
    ...
    def _insert_edge_batch(self, type_name: str, rows: list) -> None:
        ...
        with self.engine.connect() as conn:
            conn.execute(
                text(f"INSERT INTO {table_name} (src_id, dst_id, src_type, dst_type) "
    ...
    def _create_unified_views(self) -> None:
        ...
        with self.engine.connect() as conn:
            ...
            for type_name, table_name in self.node_type_tables.items():
                node_unions.append(
                    f"SELECT id, '{sanitize_table_name(type_name)}' as type, name, description, raw_json FROM {table_name}"
                )
            ...
            if node_unions:
                ...
                for type_name, table_name in self.node_type_tables.items():
                    raw_node_unions.append(
                        f"SELECT id, '{type_name}' as type, name, description, raw_json FROM {table_name}"
                    )
                conn.execute(text(f"""
    ...
    def _create_indexes(self) -> None:
        ...
        with self.engine.connect() as conn:
            ...
            for table_name in self.edge_type_tables.values():
                conn.execute(text(f"CREATE INDEX IF NOT EXISTS idx_{table_name}_src ON {table_name}(src_id)"))
                conn.execute(text(f"CREATE INDEX IF NOT EXISTS idx_{table_name}_dst ON {table_name}(dst_id)"))
    ...
    def get_schema_summary(self) -> str:
        ...
        for type_name, table_name in sorted(self.node_type_tables.items()):
            ...
            lines.append(f"    Columns: id (INTEGER PK), name (TEXT), description (TEXT), raw_json (TEXT)")
        ...
        for type_name, table_name in sorted(self.edge_type_tables.items()):
            ...
            count = result[0]["cnt"] if result else 0
            lines.append(f"  {table_name} ({count:,} rows) - {type_name}")
            lines.append(f"    Columns: src_id (INTEGER), dst_id (INTEGER), src_type (TEXT), dst_type (TEXT)")
        
        lines.append("")
        lines.append("UNIFIED VIEWS (for cross-type queries):")
        lines.append("  all_nodes - view combining all entity tables")
        lines.append("    Columns: id, type, name, description, raw_json")
